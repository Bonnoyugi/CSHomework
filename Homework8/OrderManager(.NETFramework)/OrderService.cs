...
namespace OrderManager
{
    public class OrderService
    {
        ...
        {
            currentOrderCount++;
            Order order = new Order(currentOrderCount, customer);
            return order;
        }
        ...
        {
            return orders.Where(order => order.OrderItems.Exists(orderItem => orderItem.Product.Name == product)).ToList();
        }

        public List<Order> QueryOrdersByCustomer(string customer)
        {
            return orders.Where(order => order.Customer.Name == customer).ToList();
        }
    }
}
